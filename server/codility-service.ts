...
const CODILITY_API_URL = "https://codility.com/api";
const CODILITY_API_KEY = process.env.CODILITY_API_KEY;

async function makeRequest(endpoint: string, options: any = {}) {
  const url = `${CODILITY_API_URL}${endpoint}`;
  const headers = {
    "Content-Type": "application/json",
    Authorization: `Bearer ${CODILITY_API_KEY}`,
    ...options.headers,
  };
  ...
  if (!response.ok) {
    ...
    throw new Error(`Codility API Error: ${response.status} ${response.statusText} - ${JSON.stringify(errorData)}`);
  }
...
export async function inviteCandidate(assessmentId: number, candidateEmail: string) {
  const invitation = await makeRequest(`/assessaments/${assessmentId}/invite`, {
    method: "POST",
    body: JSON.stringify({
