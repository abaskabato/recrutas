...
import fs from "fs";
import { insertCandidateProfileSchema, insertJobPostingSchema, insertChatMessageSchema, jobPostings, users, jobApplications } from "@shared/schema";
...
export async function registerRoutes(app) {
    ...
    app.post("/api/user/select-role", async (req, res) => {
        try {
            ...
            for (const job of shuffledJobs.slice(0, 15)) { // Limit to 15 matches for performance
                try {
                    ...
                    if (aiResult.score >= scoreThreshold) {
                        ...
                        const skillMatches = safeProfile.skills.filter(skill => job.skills.some(jobSkill => jobSkill.toLowerCase().includes(skill.toLowerCase()) ||
                            skill.toLowerCase().includes(jobSkill.toLowerCase()))).sort();
                        const match = {
    ...
    app.get('/api/candidate/profile', requireAuth, async (req, res) => {
        try {
            const userId = req.user.id;
    ...
    app.post('/api/candidate/profile', requireAuth, async (req, res) => {
        try {
            const userId = req.user.id;
    ...
    app.patch('/api/candidate/profile', requireAuth, async (req, res) => {
        try {
            const userId = req.user.id;
            const profileData = {
                ...req.body,
            ...
            if (candidateProfile) {
                try {
                    ...
                    for (const job of shuffledJobs) {
                        ...
                        if (usedJobIds.has(jobKey))
                            continue;
                        usedJobIds.add(jobKey);
                        if (externalMatches.length >= 10)
                            break; // Limit external matches
                        ...
                        externalMatches.push({
                            ...
                            skillMatches: candidateProfile.skills?.filter(skill => job.skills?.some(jobSkill => jobSkill.toLowerCase().includes(skill.toLowerCase()) ||
    ...
    app.get('/api/candidates/applications', requireAuth, async (req, res) => {
        try {
            const userId = req.user.id;
    ...
    // Candidate activity
    app.get('/api/candidates/activity', requireAuth, async (req, res) => {
        try {
            const userId = req.user.id;
    ...
    app.post('/api/candidates/profile/complete', requireAuth, async (req, res) => {
        try {
            const userId = req.user.id;
    ...
    app.post('/api/candidates/generate-matches', requireAuth, async (req, res) => {
        try {
            const userId = req.user.id;
            const profile = await storage.getCandidateProfile(userId);
            if (!profile) {
    ...
    app.post('/api/candidates/mark-applied/:matchId', requireAuth, async (req, res) => {
        try {
            const userId = req.user.id;
    ...
    app.get('/api/advanced-matches/:candidateId', requireAuth, async (req, res) => {
        try {
    ...
    app.put('/api/candidate/match-preferences', requireAuth, async (req, res) => {
        try {
            const userId = req.user.id;
    ...
    app.get('/api/jobs/:id/exam', requireAuth, async (req, res) => {
        try {
    ...
    app.post('/api/jobs/:id/exam/attempt', requireAuth, async (req, res) => {
        try {
            const userId = req.user.id;
            const jobId = parseInt(req.params.id);
    ...
    app.put('/api/jobs/:id/exam/attempt/:attemptId', requireAuth, async (req, res) => {
        try {
            const userId = req.user.id;
    ...
    app.get('/api/jobs/:id/chat', requireAuth, async (req, res) => {
        try {
            const userId = req.user.id;
            const jobId = parseInt(req.params.id);
    ...
    app.get('/api/jobs/:id/matches', requireAuth, async (req, res) => {
        try {
            const jobId = parseInt(req.params.id);
    ...
    app.post('/api/jobs/:id/regenerate-matches', requireAuth, async (req, res) => {
        try {
            const userId = req.user.id;
            const jobId = parseInt(req.params.id);
            // Get the job details
    ...
    app.post('/api/candidates/upload-resume', requireAuth, upload.single('resume'), async (req, res) => {
        try {
            const userId = req.user.id;
            if (!req.file) {
                return res.status(400).json({ message: "No file uploaded" });
    ...
    app.post('/api/candidates/apply/:jobId', requireAuth, async (req, res) => {
        try {
            const userId = req.user.id;
    ...
    app.post('/api/candidates/apply-external', requireAuth, async (req, res) => {
        try {
            const userId = req.user.id;
            ...
            // Log activity
            await storage.createActivityLog(userId, 'external_job_application', `Applied to external job: ${jobData.title} at ${jobData.company}`, {
                externalJobId: jobData.id,
                source,
                externalUrl,
    ...
    app.post('/api/chat/start/:matchId', requireAuth, async (req, res) => {
        try {
    ...
    app.get('/api/chat/room/:matchId', requireAuth, async (req, res) => {
        try {
            const matchId = parseInt(req.params.matchId);
    ...
    app.get('/api/live-jobs', async (req, res) => {
        try {
            ...
            if (location && typeof location === 'string' && location !== 'any') {
                allJobs = allJobs.filter(job => job.workType === 'remote' ||
    ...
    async function fetchOpenJobSources(skills, jobTitle) {
        ...
        try {
            ...
            if (jobs.length < 5) {
                try {
                    const companyJobs = await companyJobsAggregator.getAllCompanyJobs(skills ? skills.split(',').map(s => s.trim()) : undefined, 10);
        ...
        try {
            ...
            if (jobTitle && typeof jobTitle === 'string') {
                ...
                filteredJobs = filteredJobs.filter(job => {
                    ...
                    return titleKeywords.some(keyword => jobTitleLower.includes(keyword) ||
            ...
            if (skills && typeof skills === 'string') {
                ...
                filteredJobs = externalJobs.filter(job => {
                    ...
                    return skillsArray.some(skill => jobSkills.some(js => js.includes(skill)) ||
                ...
                if (filteredJobs.length === 0 && skillsArray && skillsArray.length > 0) {
                    ...
                    if (skill.includes('sales')) {
                        relevantJobs.push({ title: 'Sales Representative', company: 'SalesForce Solutions', description: 'Drive revenue growth through client acquisition and relationship management. Lead generation and closing deals.', skills: ['Sales', 'Business Development', 'CRM', 'Lead Generation'] }, { title: 'Business Development Manager', company: 'Growth Partners Inc', description: 'Identify new business opportunities and build strategic partnerships to expand market reach.', skills: ['Sales', 'Account Management', 'Strategy', 'Partnerships'] }, { title: 'Account Executive', company: 'Revenue Systems', description: 'Manage key accounts and exceed quarterly sales targets through consultative selling approach.', skills: ['Sales', 'Account Management', 'Negotiation', 'Customer Relations'] }, { title: 'Inside Sales Specialist', company: 'Digital Commerce Co', description: 'Generate leads and close deals through phone and email outreach. Build prospect pipeline.', skills: ['Sales', 'Lead Generation', 'Cold Calling', 'Email Marketing'] });
                    }
                    else if (skill.includes('design')) {
                        relevantJobs.push({ title: 'UX/UI Designer', company: 'Creative Studio Design', description: 'Design intuitive user experiences for web and mobile applications. Create wireframes, prototypes, and user flows.', skills: ['Design', 'UI/UX', 'Figma', 'User Research', 'Prototyping'] }, { title: 'Graphic Designer', company: 'Brand Agency Creative', description: 'Create visual content for marketing campaigns and brand identity. Design logos, brochures, and digital assets.', skills: ['Design', 'Graphics', 'Adobe Creative Suite', 'Branding', 'Typography'] }, { title: 'Product Designer', company: 'Innovation Design Lab', description: 'Lead product design from concept to implementation. Work with cross-functional teams to deliver user-centered solutions.', skills: ['Design', 'Product Development', 'User Research', 'Design Systems', 'Collaboration'] }, { title: 'Visual Designer', company: 'Media Design House', description: 'Develop creative assets for digital and print media. Ensure brand consistency across all visual communications.', skills: ['Design', 'Visual Communication', 'Creative Direction', 'Brand Guidelines', 'Digital Media'] });
                    }
                    else if (skill.includes('marketing')) {
                        relevantJobs.push({ title: 'Digital Marketing Specialist', company: 'Growth Marketing Agency', description: 'Execute digital campaigns across social media, Google Ads, and email marketing channels to drive customer acquisition.', skills: ['Marketing', 'Digital Marketing', 'Google Ads', 'Social Media', 'Analytics'] }, { title: 'Content Marketing Manager', company: 'Content Strategy Co', description: 'Develop content strategy and create engaging marketing materials including blogs, videos, and social posts.', skills: ['Marketing', 'Content Strategy', 'Content Creation', 'SEO', 'Copywriting'] }, { title: 'Social Media Manager', company: 'Social Media First', description: 'Manage brand presence across social media platforms. Create content calendars and engage with community.', skills: ['Marketing', 'Social Media', 'Community Management', 'Content Creation', 'Brand Management'] }, { title: 'Marketing Coordinator', company: 'Brand Solutions Inc', description: 'Support marketing initiatives and campaign execution. Coordinate events, manage vendors, and track campaign performance.', skills: ['Marketing', 'Campaign Management', 'Event Planning', 'Project Coordination', 'Analytics'] });
                    }
                    else if (skill.includes('finance')) {
                        relevantJobs.push({ title: 'Financial Analyst', company: 'Finance Corp', description: 'Analyze financial data and prepare investment recommendations. Build financial models and conduct market research.', skills: ['Finance', 'Financial Analysis', 'Excel', 'Financial Modeling', 'Investment Analysis'] }, { title: 'Accounting Specialist', company: 'Numbers Plus Accounting', description: 'Manage accounts payable, receivable, and financial reporting. Ensure compliance with accounting standards.', skills: ['Finance', 'Accounting', 'QuickBooks', 'Financial Reporting', 'Bookkeeping'] }, { title: 'Budget Analyst', company: 'Strategic Finance Group', description: 'Develop and monitor organizational budgets and forecasts. Provide variance analysis and recommendations.', skills: ['Finance', 'Budget Analysis', 'Forecasting', 'Financial Planning', 'Cost Analysis'] });
                    }
                    else if (skill.includes('hr') || skill.includes('human resources')) {
                        relevantJobs.push({ title: 'Human Resources Specialist', company: 'People First HR', description: 'Support talent acquisition and employee development programs. Handle benefits administration and compliance.', skills: ['HR', 'Talent Acquisition', 'Employee Relations', 'Benefits Administration', 'Compliance'] }, { title: 'Talent Acquisition Coordinator', company: 'Hiring Solutions Group', description: 'Source and screen candidates for various positions. Coordinate interviews and manage recruitment pipeline.', skills: ['HR', 'Recruiting', 'Talent Sourcing', 'Interview Coordination', 'Candidate Management'] }, { title: 'HR Business Partner', company: 'Strategic HR Solutions', description: 'Partner with business leaders on HR strategy and initiatives. Support organizational development and change management.', skills: ['HR', 'Business Partnership', 'Organizational Development', 'Strategy', 'Change Management'] });
                    }
                    else {
                        // Default to general positions for other skills
                        relevantJobs.push({ title: `${skill.charAt(0).toUpperCase() + skill.slice(1)} Specialist`, company: 'Professional Services Inc', description: `Apply your ${skill} expertise to drive business results and client satisfaction.`, skills: [skill.charAt(0).toUpperCase() + skill.slice(1), 'Communication', 'Problem Solving'] }, { title: `Senior ${skill.charAt(0).toUpperCase() + skill.slice(1)} Coordinator`, company: 'Excellence Partners', description: `Lead ${skill}-focused initiatives and coordinate cross-functional projects.`, skills: [skill.charAt(0).toUpperCase() + skill.slice(1), 'Project Management', 'Leadership'] });
                    }
            ...
            if (location && typeof location === 'string' && location.toLowerCase() !== 'any') {
                ...
                filteredJobs = filteredJobs.filter(job => {
                    ...
                    if (locationQuery === 'remote' && jobWorkType.includes('remote'))
                        return true;
                    if (jobWorkType.includes('remote'))
                        return true; // Always include remote jobs
            ...
            const formattedJobs = shuffledJobs.slice(0, parseInt(limit)).map((job, index) => ({
    ...
    app.get('/api/recruiter/jobs', requireAuth, async (req, res) => {
        try {
            const userId = req.user.id;
            const jobs = await storage.getJobPostings(userId);
            res.json(jobs);
    ...
    app.get('/api/applications/status', requireAuth, async (req, res) => {
        try {
            const userId = req.user.id;
            const applications = await storage.getApplicationsWithStatus(userId);
            res.json(applications);
    ...
    app.get('/api/applications/:id/intelligence', requireAuth, async (req, res) => {
        try {
            const applicationId = parseInt(req.params.id);
            const userId = req.user.id;
    ...
    app.post('/api/jobs/:id/quick-apply', requireAuth, async (req, res) => {
        try {
            const jobId = parseInt(req.params.id);
            const userId = req.user.id;
            // Check if already applied
    ...
    app.post('/api/chat/:matchId/room', requireAuth, async (req, res) => {
        try {
            const matchId = parseInt(req.params.matchId);
    ...
    app.post('/api/candidates/start-chat/:jobId', requireAuth, async (req, res) => {
        try {
            const userId = req.user.id;
            const jobId = parseInt(req.params.jobId);
    ...
    // Submit exam answers
    app.post("/api/jobs/:jobId/exam/submit", requireAuth, async (req, res) => {
        try {
            const jobId = parseInt(req.params.jobId);
    ...
    app.post('/api/talent/applications/:applicationId/track', requireAuth, async (req, res) => {
        try {
    ...
    app.patch('/api/talent/applications/:applicationId', requireAuth, async (req, res) => {
        try {
            const { applicationId } = req.params;
    ...
    app.post('/api/talent/applications/:applicationId/feedback', requireAuth, async (req, res) => {
        try {
            const { applicationId } = req.params;
    ...
    app.post('/api/talent/jobs/:jobId/rank-candidates', requireAuth, async (req, res) => {
        try {
    ...
    app.get('/api/talent/applications', requireAuth, async (req, res) => {
        try {
            const talentId = req.user.id;
    ...
    app.post('/api/talent/transparency-settings', requireAuth, async (req, res) => {
        try {
            const talentId = req.user.id;
