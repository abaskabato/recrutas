...
export class ResumeParser {
    ...
    extractEducation(text) {
        ...
        if (educationSection) {
            ...
            lines.forEach(line => {
                const hasdegree = degrees.some(degree => line.toLowerCase().includes(degree.toLowerCase()));
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (line.includes(sectionName.toLowerCase()) && line.length < 50) {
                    ...
                    for (let j = i + 1; j < lines.length; j++) {
                        ...
                        const isNextSection = sectionNames.some(name => nextLine.toLowerCase().includes(name.toLowerCase()) && nextLine.length < 50) || nextLine.match(/^[A-Z\s]{2,}$/);
                        if (isNextSection)
                            break;
