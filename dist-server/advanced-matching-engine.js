...
export class AdvancedMatchingEngine {
    matchCache = new Map();
    CACHE_DURATION = 60 * 1000; // 1 minute for fresh matches
    ...
    calculateCompatibilityFactors(criteria, job) {
        ...
        const matchingSkills = criteria.skills.filter(skill => jobSkills.some((jobSkill) => jobSkill.toLowerCase().includes(skill.toLowerCase()) ||
    ...
    calculateUrgencyScore(job) {
        ...
        if (job.applicationCount !== undefined) {
            if (job.applicationCount < 10)
                urgency += 0.2;
            else if (job.applicationCount > 100)
                urgency -= 0.1;
        }
        // Remote work bonus
    ...
    calculateLocationDistance(loc1, loc2) {
        ...
        if (!city1 || !city2)
            return 50; // Default distance for unknown cities
