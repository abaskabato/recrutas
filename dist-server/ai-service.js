...
export async function generateJobMatch(candidate, job) {
...
function analyzeExperienceSemantics(experience, jobDescription, jobTitle) {
    ...
    const commonWords = expWords.filter(word => word.length > 3 && jobWords.includes(word)).length;
...
export async function generateJobInsights(candidateProfile) {
    const insights = [];
...
function analyzeSkillGaps(currentSkills) {
    ...
    return inDemandSkills.filter(skill => !currentSkillsLower.some(cs => cs.includes(skill.toLowerCase())));
}
