...
export class HRIntegrationService {
    ...
    async syncCandidateToHR(candidateId, hrSystem) {
        try {
    ...
    async handleWebhook(hrSystem, payload, signature) {
        try {
            const config = this.configs.get(hrSystem);
    ...
    async syncToWorkday(candidate, config) {
    ...
    async fetchJobsFromWorkday(config) {
    ...
    async processWorkdayWebhook(payload) {
    ...
    async syncToGreenhouse(candidate, config) {
        const response = await fetch(`${config.baseUrl}/candidates`, {
            method: 'POST',
            headers: {
    ...
    async fetchJobsFromGreenhouse(config) {
        ...
        return data.map((job) => ({
            ...
            requirements: [],
            location: job.offices.map((office) => office.name).join(', '),
            department: job.departments.map((dept) => dept.name).join(', '),
            status: job.status === 'open' ? 'active' : 'closed'
        }));
    }
    async processGreenhouseWebhook(payload) {
    ...
    async syncToLever(candidate, config) {
        const response = await fetch(`${config.baseUrl}/candidates`, {
            method: 'POST',
            headers: {
                'Authorization': `Basic ${Buffer.from(config.apiKey + ':').toString('base64')}`,
    ...
    async fetchJobsFromLever(config) {
    ...
    async processLeverWebhook(payload) {
    ...
    // Utility methods
    transformCandidateData(dbCandidate) {
    ...
    async storeJobsInDatabase(jobs, source) {
    ...
    // Bulk operations
    async bulkSyncCandidates(hrSystem, candidateIds) {
        let success = 0;
        let failed = 0;
    ...
    async addHRSystemConfig(systemName, config) {
        this.configs.set(systemName, config);
        // In production, save to database
    }
    async removeHRSystemConfig(systemName) {
