...
- July 13, 2025. **Fixed Vercel authentication endpoints with proper serverless function structure** - Resolved 405 Method Not Allowed errors on production deployment by creating separate API files for each authentication endpoint: `api/auth/sign-up.js`, `api/auth/sign-in.js`, and `api/auth/sign-out.js`. Each endpoint now properly handles CORS, method validation, and database operations. Updated session endpoint to fix user ID mapping issue. Authentication system now fully operational on Vercel with automatic database table creation, session management, and secure cookie handling. Local development unchanged.
