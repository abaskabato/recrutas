...
- July 13, 2025. **Fixed critical Vercel deployment module resolution errors** - Resolved `ERR_MODULE_NOT_FOUND` errors by correcting import paths in all API files from non-existent `server/auth.js` to actual `server/betterAuth.js`. Fixed TypeScript schema compilation errors by removing problematic `.omit()` calls and using direct `createInsertSchema()`. Created custom `vercel-build.js` script that only builds frontend, eliminating esbuild conflicts with Vercel's serverless function handling. All API endpoints now properly import authentication module, resolving the 500 Internal Server Error on `/api/session` endpoint.
