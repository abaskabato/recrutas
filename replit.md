...
- July 7, 2025. **Database connection issue identified in production** - Supabase database connection failing in Vercel with `getaddrinfo ENOTFOUND` error. Better Auth configuration is correct, but DATABASE_URL environment variable needs to be properly set in Vercel production environment with correct Supabase connection string.
